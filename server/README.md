...
Билеты уходят по SMTP из `.env` (`SMTP_HOST` и соседние). Пока переменные пусты,
письмо не отправляется, а печатается в журнал — и заказ честно остаётся с пометкой
«письмо не ушло»: в админке видно, кому билет так и не доехал.

Перед тем как полагаться на ящик, проверьте его:

```bash
npm run mail:test -- you@example.com
```

Команда логинится на сервер, а потом шлёт настоящее письмо билета — сразу видно
и пароль, и вёрстку, и QR во вложении. В админке у оплаченного заказа есть кнопка
«Письмо»: отправить билет повторно, если покупатель говорит, что не получил.
